...
# a compressor, turbine, shaft, motor, recuperator and generator.
...
motor_ramp_up_duration = 1000.0  #1000.0
heat_ramp_up_duration = 10000.0
motor_ramp_down_duration = 1000.0  #1000.0
post_motor_time = 1000000000.0
t1 = ${motor_ramp_up_duration}
t2 = ${fparse t1 + heat_ramp_up_duration}
t3 = ${fparse t2 + motor_ramp_down_duration}
t4 = ${fparse t3 + post_motor_time}
...
L4 = ${L1}
L5 = ${L1}
L6 = ${fparse 1000*recuperator_width}
L7 = ${fparse L5 + L4 + L3}
L8 = ${L2}
...
y5 = ${y1}
y6 = ${y1}
y7 = ${fparse y6 - 1000*recuperator_width}
y8 = ${y7}
...
n_elems5 = ${n_elems1}
n_elems6 = 5
n_elems7 = ${fparse n_elems3+n_elems4+n_elems5}
n_elems8 = ${n_elems2}
...
[Functions]
  [motor_torque_fn]
    ...
    x = '0 ${t1}               ${t2}               ${t3} ${t4}'
    y = '0 ${motor_torque_max} ${motor_torque_max} 0     0'
  []
  [q_wall_fn]
    type = PiecewiseLinear
    x = '0 ${t1} ${t2} ${t3} ${t4}'
    y = '0 0 ${fparse (tot_power+recuperator_power)/(pi*D3*L3)} ${fparse (tot_power+recuperator_power)/(pi*D3*L3)} ${fparse (tot_power+recuperator_power)/(pi*D3*L3)}'    
  []
  [motor_power_fn]
    type = ParsedFunction
...
[Components]
  ...
  [junction7_8]
    type = JunctionOneToOne1Phase
    connections = 'pipe7:out pipe8:in'
  []  
  [pipe8]
    type = FlowChannel1Phase
    position = '${x8} ${y8} 0'
...
#  []
  ...
  [recuperator]
    ...
    orientation = '-1  0  0'
    position = '${fparse (x3+x8)/2} ${fparse (y3+y8)/2} 0'
    length = ${fparse (L2+L8)/2}
    widths = ${recuperator_width}
    n_elems = ${fparse (n_elems2+n_elems8)/2}
    n_part_elems = 2
    names = recuperator
    materials = steel
  ...
  [heat_transfer_cold_leg]
    ...
    hs = recuperator
    hs_side = OUTER
    Hw = ${fparse 42570.03199519359} #4865.708274 #10000
...
#    show = 'shaft_speed p_ratio_comp p_ratio_turb compressor:pressure_ratio turbine:pressure_ratio'
    show = 'shaft_speed p_ratio_comp compressor:pressure_ratio turbine:pressure_ratio'
  []
[]
